var Element_buttonSendDocumentMail = (function (_super) {
    ...
    function Element_buttonSendDocumentMail() {
        ...
        this.configTemplate =
            '<script id="elementConfiguratorButtonSendDocumentMail-Template" type="text/x-kendo-template">' +
                ...
                '</div>' +
            '</script>';
        $(document.body).append(this.configTemplate);
    ...
    Element_buttonSendDocumentMail.prototype.onClick = function (e) {
        ...
        this.hoverPanel = new ecspand.Controls.HoverPanel($("#" + _this.id), { closeOnClick: true });
        ...
        this.downloadFileData(
            ...
            .then(function (data) {
                ...
                var fileName = _this.template.ctx.get_listItem().get_item("FileLeafRef"),
                    ...
                    mailData = {
                        personalizations: [{ "to": [{ "email": _this.formViewModel.get("email") }] }],
                        from: { "email": _this.formViewModel.get("fromEmail") },
                        subject: _this.getDefaultViewModel().get("messageTitle"),
                        content: [{ type: "text/plain", value: _this.getDefaultViewModel().get("message") }],
                        attachments: [
                            {
                                content: data,
                                filename: fileName,
