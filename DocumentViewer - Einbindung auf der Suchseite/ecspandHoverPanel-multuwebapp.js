        ...
        if ($grid.length > 0) {
            ...
            hpTemp.showOnMouseOverDelayed(400, function (e) {
                ...
                if (isDocumentType) {
                    ...
                    // Für den Fall, das die Elemente aus einer anderen SiteCollection abgefragt werden sollen, muss zuvor die webUrl des Elementes Ermittelt werden
                    ECSpand.Core.Webservices.ScriptService.getWebUrlPromise(_spPageContextInfo.siteAbsoluteUrl, siteId, webId).then(function (webUrl) {
                        
                        // Workaround für einen SharePoint Bug bei dem ein EventHandler dazu führt, dass keine WebApplication-übergreifenden Requests ausgeführt werden können
                        oldEvents = Sys.Net.WebRequestManager._events;
                        Sys.Net.WebRequestManager._events = new Sys.EventHandlerList;

                        // Mit Hilfe der zuvor ermittelten Informationen kann nun das benötigte ElementContext Object erstellt werden
                        return ecspand.SPObjectContainer.getCurrent().getElementContextWithWebID(webUrl, new SP.Guid(webId), new SP.Guid(listId), parseInt(itemId));
                        ...
                        dv.setDocument(ctx).done(function() {               
                            Sys.Net.WebRequestManager._events = oldEvents;
                        });
